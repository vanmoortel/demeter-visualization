...
import {Typography, Layout, Select, Switch, Input} from 'antd'
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!graphData?.nodes?.length) {
        setGraphData({
          links: [], nodes: Object.keys(guild.users)
              .map((k) => ({
                id: k,
          ...
          <div style={{ display: 'flex', flexDirection: 'row', height: "100%", alignItems: 'baseline', justifyContent: 'flex-start', gap: '10px 30px'}}>
            <div style={{ display: 'flex', flexDirection: 'column', width: '20%', rowGap: '10px'}}>
              ...
              <Select value={`${roundId}`} style={{width: 120}} onChange={(value) => setRoundId(parseInt(value))}>
                {
                  Array(guild?.rounds?.length || 0).fill(0).map((_, i) => (
                      <Select.Option
                          value={`${i}`}>{`${moment(guild?.rounds[i]?.startDate).format("MMM Do YY")}`}</Select.Option>
                  ))
                }
              </Select>
            ...
            {usersRep.filter(u => u.reputations[roundId] > 1)?.length && (
                <div style={{ display: 'flex', flexDirection: 'column', height: '20vh', width: '80%', flexWrap: 'wrap', gap: '4px 4px', overflow: 'scroll'}}>
                  {usersRep.filter(u => u.reputations[roundId] > 1)
                      .map((u, i) => (
                          <Typography.Title key={`user-${i}`} level={5} style={{ fontSize: '0.8em', width: '200px', margin: '0px'}}>
                            {`${i+1} - ${u.displayName?.slice(0, 12) || 'Missing'} => ${new Intl.NumberFormat('en-US', {maximumFractionDigits: 2}).format(u.reputations[roundId])}`}
                          </Typography.Title>
                      ))}
                </div>
            ) }
